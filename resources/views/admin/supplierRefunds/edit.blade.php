...
@section('title', 'Update Supplier Refund')
@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>Update Supplier Refund</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ route('admin.supplier-refunds.index') }}">Supplier Refunds</a></li>
                <li class="breadcrumb-item active">Update Supplier Refund</li>
            </ol>
...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.supplier-refunds.update',['supplier_refund'=>$refund->id]) }}" method="POST" enctype="multipart/form-data" id="admin-form">
                        @method('PUT')
                        @csrf
                        @if (count($errors) > 0)
                                    ...
                                    <select name="supplier_id" class="select2 form-control" id="role" readonly>
                                        @if(!empty($suppliers))
                                            @foreach($suppliers as $supplier)
                                                <option value="{{ $supplier->id }}" {{ $refund->supplier_id == $supplier->id ? 'selected' : '' }}>
                                                    {{ $supplier->name }}
                                                </option>
                                            @endforeach
                                        @else
