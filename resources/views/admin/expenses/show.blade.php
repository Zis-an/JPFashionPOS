...
@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>View Expense - {{ $expense->title ?? '' }}</h1>
        </div>
        <div class="col-sm-6">
                ...
                <div class="card-body">
                    ...
                    <table class="table table-bordered w-100 text-left">
                        ...
                        <tr>
                            ...
                            <td>{{ strip_tags($expense->details ?? '') }}</td>
                        </tr>
                        <tr>
                        ...
                        <div class="my-3">
                            <legend>Expense's Photos</legend>
                        </div>
                        <div id="image-preview-container" class="form-group mb-2 d-flex flex-wrap">
                            @if($expense->images)
                                <img src="{{ asset($expense->images) }}" alt="Expense Photo" class="selected-image">
                            @else
                    ...
                    <div class="row mt-5 mb-4">
                        <div class="col-md-12">
                            ...
                            @if($activities->isEmpty())
                                <p>No activities found for this expense.</p>
                            @else
...
@section('js')
    ...
    <!-- Status Change -->
    <script>
        function changeStatus(expenseId, status) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `/admin/expenses/${expenseId}/change-status`; // Include admin prefix
            ...
            // Add method field for PUT or PATCH
            const methodInput = document.createElement('input');
            methodInput.type = 'hidden';
            methodInput.name = '_method';
            methodInput.value = 'PATCH'; // or PUT
        ...
        document.querySelector('.btn-primary').addEventListener('click', function(e) {
            e.preventDefault();
            changeStatus({{ $expense->id }}, 'approved');
        });

        document.querySelector('.btn-secondary').addEventListener('click', function(e) {
            e.preventDefault();
            changeStatus({{ $expense->id }}, 'rejected');
        });
    </script>
@stop
