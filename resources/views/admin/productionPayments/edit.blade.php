...
@section('title', 'Update Production House Payment')
@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>Update Production House Payment</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ route('admin.production-payment.index') }}">Production House Payments</a></li>
                <li class="breadcrumb-item active">Update Production House Payment</li>
            </ol>
...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.production-payment.update',['production_payment'=>$payment->id]) }}" method="POST" enctype="multipart/form-data" id="admin-form">
                        @method('PUT')
                        @csrf
                        @if (count($errors) > 0)
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="house_id">Select Production House <span class="text-danger font-weight-bolder">*</span></label>
                                    <select name="house_id" class="select2 form-control" id="role" readonly>
                                        @if(!empty($suppliers))
                                            @foreach($houses as $house)
                                                <option value="{{ $house->id }}" {{ $payment->house_id == $house->id ? 'selected' : '' }}>
                                                    {{ $house->name }}
                                                </option>
                                            @endforeach
                                        @else
                                            <option>No production house available</option>
                                        @endif
                                    </select>
                                </div>
                            </div>
