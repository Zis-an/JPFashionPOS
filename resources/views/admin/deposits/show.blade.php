...
@section('title', 'View Deposit')

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>View Deposit - {{ $deposit->account->name }}</h1>
        </div>
        <div class="col-sm-6">
            ...
            <div class="card">
                <div class="card-body">
                        ...
                        @endif
                            <table class="table table-bordered w-100 text-left">
                                ...
                                <tr>
                                    ...
                                    <td>{{ $deposit->amount ?? '' }} BDT</td>
                                </tr>
                                <tr>
                                    <th style="width: 30%;">Details</th>
                                    <td>{{ strip_tags($deposit->notes) ?? '' }}</td>
                                </tr>
                                <tr>
                            ...
                            <div class="my-3">
                                <legend>Deposit's Photos</legend>
                            </div>
                            <div id="image-preview-container" class="form-group mb-2 d-flex flex-wrap">
                                @if($deposit->image)
                                    <img src="{{ asset($deposit->image) }}" alt="Deposit Photo" class="selected-image">
                                @else
                        ...
                        <form action="{{ route('admin.deposits.destroy', $deposit->id) }}" method="POST">
                            @method('DELETE')
                            @csrf
                            <a href="{{route('admin.deposits.index')}}" class="btn btn-success" >Go Back</a>
                            @can('deposits.update')
                                <a href="{{route('admin.deposits.edit',['deposit'=>$deposit->id])}}" class="btn btn-warning "><i class="fa fa-pen"></i> Edit</a>
                            @endcan
                            @can('deposits.delete')
                            ...
                            <div class="row mt-4 mb-4">
                                <div class="col-md-12">
                                    ...
                                    @if($activities->isEmpty())
                                        <p>No activities found for this deposit.</p>
                                    @else
