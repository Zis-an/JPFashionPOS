...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.deposits.update',['deposit'=>$deposit->id]) }}" method="POST" enctype="multipart/form-data" id="admin-form">
                        @method('PUT')
                        @csrf
                        @if (count($errors) > 0)
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="amount" name="amount" value="{{ $deposit->amount }}" type="number" class="form-control" placeholder="Enter amount">
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-group">
                                    <label for="details">Notes</label>
                                    <textarea name="notes" id="summernote" class="form-control">{{ $deposit->notes }}</textarea>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="photo">Select photo</label>
                                    <input name="old_photo" value="{{ asset($deposit->image) ?? '' }}" class="d-none">
                                    <input name="photo" type="file" class="form-control" id="photo">
                                </div>
                                @if($deposit->image)
                                    <div class="form-group mb-2">
                                        <img src="{{ asset($deposit->image) }}" alt="Selected Image" id="selected-image">
                                    </div>
                                @endif
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        function displayExistingPhoto() {
            const existingPhotoUrl = '{{ asset($deposit->image) }}';
            if (existingPhotoUrl) {
