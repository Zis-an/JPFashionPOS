...
@section('title', 'View Customer Refund')
@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>View Customer Refund - {{ $refund->customer->name }}</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{route('admin.customer-refunds.index')}}">Customer Refunds</a></li>
                <li class="breadcrumb-item active">View Customer Refund</li>
            </ol>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table table-bordered w-100 text-left">
                        ...
                        <tr>
                            <th style="width: 30%;">Refund By</th>
                            <td>{{ strip_tags($refund->refund_by) ?? '' }}</td>
                        </tr>
                        <tr>
                    ...
                    <div class="my-3">
                        <legend>Customer Refund's Photos</legend>
                    </div>
                    <div id="image-preview-container" class="form-group mb-2 d-flex flex-wrap">
                        @if($refund->image)
                            <img src="{{ asset($refund->image) }}" alt="Payment Photo" class="selected-image">
                        @else
                    ...
                    <form action="{{ route('admin.customer-refunds.destroy', $refund->id) }}" method="POST">
                        @method('DELETE')
                        @csrf
                        <a href="{{route('admin.customer-refunds.index')}}" class="btn btn-success" >Go Back</a>
                        @can('customerRefunds.update')
                            <a href="{{route('admin.customer-refunds.edit',['customer_refund'=>$refund->id])}}" class="btn btn-warning ">
