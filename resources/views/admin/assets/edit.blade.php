...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.assets.update',['asset'=>$asset->id]) }}" method="POST" enctype="multipart/form-data" id="admin-form">
                        @method('PUT')
                        @csrf
                        @if (count($errors) > 0)
                        ...
                        <div class="row">
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="details" id="summernote" class="form-control">{{ $asset->details ?? '' }}</textarea>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="photo">Select photo</label>
                                    <input name="old_photo" value="{{ asset($asset->images) ?? '' }}" class="d-none">
                                    <input name="photo" type="file" class="form-control" id="photo">
                                </div>
                                @if($asset->images)
                                    <div class="form-group mb-2">
                                        <img src="{{ asset($asset->images) }}" alt="Selected Image" id="selected-image">
                                    </div>
                                @endif
...
<script>
    $(document).ready(function() {
        ...
        function displayExistingPhoto() {
            const existingPhotoUrl = '{{ asset($asset->images) }}';
            if (existingPhotoUrl) {
