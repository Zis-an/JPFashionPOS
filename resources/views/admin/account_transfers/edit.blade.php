...
@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>Update Account Transfer</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ route('admin.account-transfers.index') }}">Transfers</a></li>
...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.account-transfers.update',['account_transfer'=>$transfer->id]) }}" method="POST"
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="from_account_id">Select Account <span class="text-danger font-weight-bolder">*</span></label>
                                    <select name="from_account_id" class="select2 form-control" id="role" required>
                                        @if(!empty($accounts))
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="to_account_id">Select Account <span class="text-danger font-weight-bolder">*</span></label>
                                    <select name="to_account_id" class="select2 form-control" id="role" required>
                                        @if(!empty($accounts))
                                            @foreach($accounts as $account)
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="amount" name="amount" value="{{ $transfer->amount }}" type="number" class="form-control" placeholder="Enter amount" required>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-group">
                                    <label for="details">Notes</label>
                                    <textarea name="notes" id="summernote" class="form-control">{{ strip_tags($transfer->notes) }}</textarea>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="photo">Select photo</label>
                                    <input name="old_photo" value="{{ asset($transfer->image) ?? '' }}" class="d-none">
                                    <input name="photo" type="file" class="form-control" id="photo">
                                </div>
                                @if($transfer->image)
                                    <div class="form-group mb-2">
                                        <img src="{{ asset($transfer->image) }}" alt="Selected Image" id="selected-image">
                                    </div>
                                @endif
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        function displayExistingPhoto() {
            const existingPhotoUrl = '{{ asset($transfer->image) }}';
            if (existingPhotoUrl) {
