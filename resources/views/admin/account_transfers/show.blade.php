...
@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>View Account Transfer - {{ $transfer->amount }} /-</h1>
        </div>
        <div class="col-sm-6">
                ...
                <div class="card-body">
                        ...
                        @endif
                            <table class="table table-bordered w-100 text-left">
                                <tr>
                                    <th style="width: 30%;">Debited Account</th>
                                    <td>{{ $transfer->fromAccount->name ?? 'N/A' }}</td>
                                </tr>
                                <tr>
                                    <th style="width: 30%;">Credited Account</th>
                                    <td>{{ $transfer->toAccount->name ?? 'N/A' }}</td>
                                </tr>
                                <tr>
                                    <th style="width: 30%;">Amount</th>
                                    <td>{{ $transfer->amount ?? '' }} BDT</td>
                                </tr>
                                <tr>
                                    <th style="width: 30%;">Details</th>
                                    <td>{{ strip_tags($transfer->notes) ?? '' }}</td>
                                </tr>
                                <tr>
                            ...
                            <div class="my-3">
                                <legend>Transfer's Photos</legend>
                            </div>
                            <div id="image-preview-container" class="form-group mb-2 d-flex flex-wrap">
                                @if($transfer->image)
                                    <img src="{{ asset($transfer->image) }}" alt="transfer Photo" class="selected-image">
                                @else
                        ...
                        <form action="{{ route('admin.account-transfers.destroy', $transfer->id) }}" method="POST">
                            @method('DELETE')
                            @csrf
                            <a href="{{route('admin.account-transfers.index')}}" class="btn btn-success" >Go Back</a>
                            @can('transfers.update')
                                <a href="{{route('admin.account-transfers.edit',['account_transfer'=>$transfer->id])}}" class="btn btn-warning "><i class="fa fa-pen"></i> Edit</a>
                            @endcan
