...
@section('title', 'View Product Stock Transfer')
@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>View Product Stock Transfer - {{ \Carbon\Carbon::parse($transfer->date)->format('F j, Y') ?? '' }}</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{route('admin.product-stock-transfers.index')}}">Product Stock Transfers</a></li>
                <li class="breadcrumb-item active">View Product Stock Transfer</li>
            </ol>
            ...
            <div class="card">
                <div class="card-body">
                        ...
                        <table class="table-bordered table">
                            ...
                            @foreach($transfer->productStocks as $productStock)
                                <tr>
                                    ...
                                    <td>{{ $productStock->pivot->quantity }}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
                    </div>
                    <a href="{{ route('admin.product-stock-transfers.index') }}" class="btn btn-success" >Go Back</a>
                    @can('productStockTransfers.update')
                        <a href="{{ route('admin.product-stock-transfers.edit',['product_stock_transfer'=>$transfer->id]) }}" class="btn btn-warning ">
                    ...
                    <form class="d-inline" action="{{ route('admin.product-stock-transfers.destroy', $transfer->id) }}" method="POST">
                        @method('DELETE')
                        @csrf
                        @can('productStockTransfers.delete')
