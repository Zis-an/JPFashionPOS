    ...
    <div class="row mb-2">
        ...
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">Update Withdraw</li>
            </ol>
...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.withdraws.update',['withdraw'=>$withdraw->id]) }}" method="POST" enctype="multipart/form-data" id="admin-form">
                        @method('PUT')
                        @csrf
                        @if (count($errors) > 0)
                        ...
                        <div class="row">
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="notes" id="summernote" class="form-control">{{ strip_tags($withdraw->notes) }}</textarea>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="photo">Select photo</label>
                                    <input name="old_photo" value="{{ asset($withdraw->image) ?? '' }}" class="d-none">
                                    <input name="photo" type="file" class="form-control" id="photo">
                                </div>
                                @if($withdraw->image)
                                    <div class="form-group mb-2">
                                        <img src="{{ asset($withdraw->image) }}" alt="Selected Image" id="selected-image">
                                    </div>
                                @endif
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        function displayExistingPhoto() {
            const existingPhotoUrl = '{{ asset($withdraw->images) }}';
            if (existingPhotoUrl) {
