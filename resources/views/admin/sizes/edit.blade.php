...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.sizes.update',['size'=>$size->id]) }}" method="POST" enctype="multipart/form-data" id="admin-form">
                        @method('PUT')
                        @csrf
                        @if (count($errors) > 0)
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="name" name="name" value="{{ $size->name ?? '' }}" class="form-control" placeholder="Enter size name" required>
                                </div>
                            </div>
                        </div>
                        @can('sizes.update')
