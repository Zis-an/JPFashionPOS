...
@section('title', 'Update Raw Material Stock')
...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.raw-material-stocks.update',['raw_material_stock'=>$stock->id]) }}" method="POST" enctype="multipart/form-data" id="admin-form">
                        @method('PUT')
                        @csrf
                        @if (count($errors) > 0)
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="material_id" name="raw_material_id" class="select2 form-control">
                                        @foreach($materials as $material)
                                            <option value="{{ $material->id }}" {{ $stock->raw_material_id == $material->id ? 'selected' : '' }}>
                                                {{ $material->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="quantity" type="number" value="{{ $stock->quantity }}" name="quantity" class="form-control" placeholder="Enter quantity">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="color_id" name="color_id" class="select2 form-control">
                                        @foreach($colors as $color)
                                            <option value="{{ $color->id }}" {{ $stock->color_id == $color->id ? 'selected' : '' }}>
                                                {{ $color->color_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="brand_id" name="brand_id" class="select2 form-control">
                                        @foreach($brands as $brand)
                                            <option value="{{ $brand->id }}" {{ $stock->brand_id == $brand->id ? 'selected' : '' }}>
                                                {{ $brand->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="size_id" name="size_id" class="select2 form-control">
                                        @foreach($sizes as $size)
                                            <option value="{{ $size->id }}" {{ $stock->size_id == $size->id ? 'selected' : '' }}>
                                                {{ $size->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="price" type="number" name="price" value="{{ $stock->price }}" class="form-control" placeholder="Enter price">
                                </div>
                            </div>
                        </div>
                        <div class="row">
